...
fn full_legal_lifecycle_is_journaled_in_order_and_verifies() {
    ...
    // One genesis + three transition records. Genesis is creation; later records
    // are explicit status transitions so auditors do not confuse them.
    assert_eq!(
        event_kinds(&session),
        vec![
            "session_created",
    ...
    let transitions: Vec<(SessionStatus, SessionStatus)> = session
        .journal()
        .records()
        .iter()
        .filter_map(|record| match &record.event {
            JournalEvent::SessionStatusChanged { from, to, .. } => Some((from.clone(), to.clone())),
            _ => None,
        })
        .collect();
    assert_eq!(
