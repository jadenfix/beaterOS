...
| NVIDIA CUDA Best Practices Guide and CUDA Graphs documentation: https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/index.html and https://docs.nvidia.com/cuda/cuda-programming-guide/04-special-topics/cuda-graphs.html | Transfer minimization, pinned-memory caution, batching, async overlap, and graph reuse for launch-overhead-heavy workloads | Requires local profiling; pinned memory is scarce and graph complexity must be justified |
...
| ROCm HIP performance and GPU isolation docs: https://rocm.docs.amd.com/projects/HIP/en/latest/how-to/performance_guidelines.html and https://rocm.docs.amd.com/en/latest/conceptual/gpu-isolation.html | AMD GPU copy/queue profiling guidance and isolation warning that environment-variable device hiding is insufficient for untrusted work | ROCm-specific; container/VM isolation still needs local threat review |
| Linux VFIO documentation: https://docs.kernel.org/driver-api/vfio.html | DMA/IOMMU isolation model for assigning devices safely to untrusted or isolated workloads | Linux-specific; not a macOS accelerator path |
...
| OpenXLA XProf and roofline docs: https://openxla.org/xprof/tensorflow_profiling and https://openxla.org/xprof/roofline_model | Host/device traces, queue/device timelines, and memory-bound vs compute-bound accelerator diagnosis | Tooling availability depends on framework/backend |
| Google Cloud TPU documentation, architecture docs, and performance guide: https://docs.cloud.google.com/tpu/docs and https://docs.cloud.google.com/tpu/docs/system-architecture-tpu-vm and https://docs.cloud.google.com/tpu/docs/performance-guide | TPU as custom ASIC/pod resource, plus input pipeline, sharding, batch, memory, and pinned-buffer performance constraints | Cloud/provider-specific; APIs and generations change |
| Groq LPU architecture, latency, and rate-limit documentation: https://groq.com/lpu-architecture and https://console.groq.com/docs/production-readiness/optimizing-latency and https://console.groq.com/docs/rate-limits | Deterministic token-generation silicon, low-jitter inference, provider telemetry, and rate-limit backpressure as a distinct accelerator/provider lane | Vendor-specific claims need benchmarked local validation |
| Apple Metal, Metal resource options, Core ML typed execution, and Core ML performance docs: https://developer.apple.com/metal/ and https://developer.apple.com/library/archive/documentation/3DDrawing/Conceptual/MTLBestPracticesGuide/ResourceOptions.html and https://apple.github.io/coremltools/docs-guides/source/typed-execution.html and https://developer.apple.com/videos/play/wwdc2023/10049/ | Local GPU, unified-memory costs, resource storage modes, Core ML CPU/GPU/Neural Engine placement, precision behavior, async prediction, and concurrency pressure | Public low-level access varies by framework and entitlement; placement may be opaque |
...
| Cargo `rust-version` manifest documentation: https://doc.rust-lang.org/cargo/reference/rust-version.html | MSRV/support-floor semantics for the repo's `rust-version = "1.93"` declaration | `rust-version` is not a latest-stable pin |
...
| Clang C and C++ status pages: https://clang.llvm.org/c_status.html and https://clang.llvm.org/cxx_status.html | C/C++ standard support checks before claiming C23, C++23, C++26, or Apple Clang behavior | Upstream Clang support does not automatically describe Apple Clang or vendor toolchains |
...
| AdaCore SPARK overview and user guide: https://www.adacore.com/languages/spark and https://docs.adacore.com/spark2014-docs/html/ug/ | Formal verification language option for narrow proof experiments | Not a default implementation language; needs reviewer, toolchain, FFI, and macOS plans |
| Rust inline assembly reference and `core::arch` docs: https://doc.rust-lang.org/reference/inline-assembly.html and https://doc.rust-lang.org/core/arch/index.html | Assembly/intrinsics boundary review, feature detection, portability, and scalar fallback requirements | Architecture-specific and non-portable by design |
