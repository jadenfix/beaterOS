...
| `trace export` | Export a full schema-shaped trace/action bundle for one live session. |
...
`trace export --session <id>` emits the full replay artifact for one session in
the shape of `contracts/schema/trace-bundle.schema.json`: session, grants,
payment mandates, approvals, simulations, manifests, policy decisions,
...
hashes rather than recomputing a presentation-only chain. This is a full
